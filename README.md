...
いっぽうSPRESENSEでは、プログラムはコードや読み出し専用のデータを含めてすべて、FlashメモリからRAM上に展開されてRAM上で実行されます。Flashメモリはあくまでファイルシステムで管理されるストレージという扱いです。そのため、SPRESENSEでu8g2の日本語フォントを使うと、数百キロバイトものRAMを使用することになります。1.5メガバイトものRAMを持つSPRESENSEとはいえ、これは少々不都合です。
